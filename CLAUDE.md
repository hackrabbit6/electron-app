...
- CI (`.github/workflows/ci.yml`) runs `typecheck` + `build` across all four framework variants on push/PR. It sets `ELECTRON_SKIP_BINARY_DOWNLOAD=1` (binary not needed for typecheck/build — vite externalizes electron). The post-scaffold `pnpm install` uses `--no-frozen-lockfile`: scaffold rewrites per-framework deps, but the committed lockfile is react's, so CI's default frozen install would fail otherwise. Verified green on GitHub Actions.
